...
const { Subline, Sequelize } = require("../../models");
...
const add = async function add(req, res, next) {
    ...
    try {
        let model = await Subline.findOne({ where: { title: req.body.title, lineId } });
        if (model) {
...
const update = async function update(req, res, next) {
    ...
    try {
        let model = await Subline.findOne({
            where: {
        ...
        model = await Subline.findByPk(modelID);
        if (!model) {
...
const get = async function get(req, res, next) {
    ...
    try {
        const model = await Subline.findByPk(modelID);
        if (!model) {
            res.sendError("notFound");
            return;
        }
