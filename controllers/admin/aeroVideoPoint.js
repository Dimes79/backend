...
const { AeroVideoPoint, Sequelize } = require("../../models");
...
const getList = async function getList(req, res, next) {
    ...
    try {
        const { rows, count } = await AeroVideoPoint.findAndCountAll({
            where: {
                status: "ACTIVE",
                lineId,
            },
...
const add = async function add(req, res, next) {
    ...
    try {
        let model = await AeroVideoPoint.findOne({ where: { title: req.body.title, lineId } });
        if (model) {
...
const update = async function update(req, res, next) {
    ...
    try {
        let model = await AeroVideoPoint.findOne({
            where: {
        ...
        model = await AeroVideoPoint.findByPk(modelID);
        if (!model) {
...
const get = async function get(req, res, next) {
    ...
    try {
        const model = await AeroVideoPoint.findByPk(modelID);
        if (!model) {
            res.sendError("notFound");
            return;
        }
